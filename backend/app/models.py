...
class User(Base):
    ...
    referral_codes = relationship("ReferralCode", foreign_keys="ReferralCode.referrer_id", back_populates="referrer")
...
# NWU token reward constants for the affiliate programme
NWU_REWARD_PER_REFERRAL = 50.0
SUBSCRIPTION_REWARD_PERCENT = 5.0
AFFILIATE_REFERRAL_THRESHOLD = 10
AFFILIATE_REVENUE_SHARE_PERCENT = 10.0
...
class ReferralCode(Base):
    """A unique referral code owned by a user, used in the full affiliate programme."""
    __tablename__ = "referral_codes"

    id = Column(Integer, primary_key=True, index=True)
    code = Column(String(20), unique=True, nullable=False, index=True, default=generate_referral_code)
    referrer_id = Column(Integer, ForeignKey("users.id"), nullable=False, index=True)
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime, default=datetime.utcnow)

    referrer = relationship("User", foreign_keys=[referrer_id], back_populates="referral_codes")
    events = relationship("ReferralEvent", back_populates="referral_code")


class ReferralEvent(Base):
    """An event generated by a referral (signup, subscription payment, reward claim)."""
    ...
    event_type = Column(String(50), nullable=False, index=True)  # signup, subscription
    nwu_reward = Column(Float, default=0.0)
    subscription_amount = Column(Float, nullable=True)
    status = Column(String(20), default="pending", index=True)  # pending, completed, claimed
    created_at = Column(DateTime, default=datetime.utcnow)
    completed_at = Column(DateTime, nullable=True)
